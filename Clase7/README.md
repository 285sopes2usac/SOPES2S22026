...
El problema de los Filósofos Gastronómicos (Dining Philosophers) es el ejemplo clásico por excelencia para demostrar los riesgos de un Deadlock (bloqueo mutuo) en la programación concurrente.

En este problema, 5 filósofos se sientan en una mesa redonda con 5 tenedores. Para comer, cada uno necesita dos tenedores (el de su izquierda y el de su derecha). Si todos toman el tenedor de su izquierda al mismo tiempo, se cumple la condición de Espera circular, dejándolos bloqueados permanentemente porque nadie soltará su tenedor hasta tener el segundo (condición de Retener y esperar).

Para evitar este deadlock e implementar una solución segura, podemos usar Mutexes para representar cada tenedor. La estrategia para evitar que el sistema colapse será romper la espera circular: haremos que los filósofos pares tomen primero el tenedor izquierdo y los impares el derecho.
